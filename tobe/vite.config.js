...
function htmlIncludePlugin() {
  return {
    ...
    transformIndexHtml(html) {
      ...
      content = content.replace(/<link\s+[^>]*?href=["']([^"']+\.scss)["'][^>]*?>/gi, (match, p1) => {
        let scssSrc = p1;
        if (scssSrc.startsWith('../')) {
          scssSrc = '/src/' + scssSrc.replace(/^\.\.\//, '');
        } else if (scssSrc.startsWith('./')) {
          scssSrc = '/src/' + scssSrc.replace(/^\.\//, '');
        } else if (!scssSrc.startsWith('/')) {
          scssSrc = '/' + scssSrc;
        }
        return `<script type="module" src="${scssSrc}"></script>`;
      });
  ...
  return {
    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        ...
        // 1. 루트 접근 시 index.html 또는 guide/guide.html 서빙
        if (url === '/' || url === '/index.html') {
          const indexPath = resolve(__dirname, 'src/index.html');
          const guidePath = resolve(__dirname, 'src/guide/guide.html');
          const targetPath = fs.existsSync(indexPath) ? indexPath : guidePath;
          if (fs.existsSync(targetPath)) {
            let html = fs.readFileSync(targetPath, 'utf-8');
            const reqPath = fs.existsSync(indexPath) ? '/src/index.html' : '/src/guide/guide.html';
            html = await server.transformIndexHtml(reqPath, html);
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      ...
      'abstracts': resolve(__dirname, 'src/styles/abstracts'),
      'components': resolve(__dirname, 'src/styles/components'),
      'base': resolve(__dirname, 'src/styles/base'),
      'origin': resolve(__dirname, 'src/styles/origin'),
      'offering': resolve(__dirname, 'src/styles/offering'),
    ...
    preprocessorOptions: {
      scss: {
        loadPaths: [
          ...
          resolve(__dirname, 'src/styles/abstracts'),
